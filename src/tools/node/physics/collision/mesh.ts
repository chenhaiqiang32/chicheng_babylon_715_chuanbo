...
export class CollisionMesh extends Mesh implements ICollisionMesh {
	...
	/** 是否显示完整网格（true=完整，false=简化） */
	private _showFullMesh: boolean = false;
	
	/** 是否使用简化线框模式 */
	private _usesSimplifiedWireframe: boolean = false;
	...
	public get usesSimplifiedWireframe(): boolean {
		return this._usesSimplifiedWireframe;
	}
	 ...
	 * 
	 * @remarks
	 * - 简化线框模式：只控制线框可见性（主网格始终隐藏以提升性能）
	 * - 完整网格模式：控制主网格和所有实例的可见性
	 */
	public setVisibility(visible: boolean): void {
		if (this._usesSimplifiedWireframe) {
			// 简化线框模式：只控制线框可见性
			if (this._linesMesh) {
	...
	/**
	 * 切换显示完整网格或简化线框
	 * @description 仅对支持简化线框的形状（sphere、cylinder、capsule）有效
	 * @param show - true=显示完整边线，false=显示简化线框
	 * 
	 * @remarks
	 * - 完整网格：从几何体提取所有边线，更精确但性能较低
	 * - 简化线框：预定义关键线条（如球体的3个圆环），性能更好
	 * - 更新时会重新创建线框网格而非修改顶点数据（因为线条数量可能变化很大）
	 */
	...
	private _updateWireframeLines(): void {
		...
		// 根据模式获取线条数据
		const lines = this._showFullMesh 
			? this._extractEdgesFromGeometry()  // 完整模式：提取所有边线
			: this._getSimplifiedWireframeLines();  // 简化模式：预定义关键线条

		if (lines.length === 0) {
			console.warn('[CollisionMesh] No lines to update for shape:', this.shape.type);
			return;
		}
		...
		try {
			...
			console.error('[CollisionMesh] Failed to update wireframe lines:', error);
		}
	}
	...
	private _configureLinesMeshMaterial(): void {
		...
		const greenColor = new Color3(0, 1, 0);  // 统一的绿色
		...
		if (this._linesMesh.material) {
			...
			this._linesMesh.material.transparencyMode = 2;  // ALPHA_BLEND
	...
	private _disposeOldGeometry(): void {
		...
		// 清理主网格的顶点缓冲区 - 这是关键！
		// 必须在释放几何体之前清理，否则从低细分切换到高细分时缓冲区不会正确更新
	...
	private _rebuildCollisionMesh(): void {
		...
		// 关键：强制计算世界矩阵，确保几何体顶点数据和变换完全同步到 GPU 渲染队列前
		this.computeWorldMatrix(true);
	...
	private async _createConvexHullCollisionMesh(sourceMesh: AbstractMesh): Promise<void> {
		...
		if (this.geometry) {
			...
			// 隐藏原始网格，只显示线框
			this.isVisible = false;
			this._usesSimplifiedWireframe = true;
		}
	} catch (error) {
		// 静默处理错误
		console.warn("Failed to create convex hull collision:", error);
	}
}
	...
	private async _createMeshCollisionMesh(sourceMesh: AbstractMesh): Promise<void> {
		...
		// 静默处理错误
		console.warn("Failed to create mesh collision:", error);
	}
}
	...
	private _createDualWireframe(): void {
		...
		try {
			...
			console.error("Failed to create dual wireframe:", error);
		}
	}
	...
	private _createSimplifiedWireframe(): void {
		...
		if (points.length === 0) {
			console.warn(`[CollisionMesh] No wireframe data for type: ${type}`);
			return;
		}
		...
		try {
			...
			console.error("Failed to create simplified wireframe:", error);
	...
	private _createCapsuleWireframe(): Vector3[][] {
		...
		// 根据 axis 旋转点集，保持和几何体 orientation 一致（符号不影响对称形状的视觉对齐）
