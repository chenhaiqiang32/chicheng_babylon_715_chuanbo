...
function changeColor(color: string) {
    const newValue = Color3.FromHexString(color)
...
const onFinish = () => {
    if (value.value !== oldValue) {
        ...
        registerPropertyUndoRedo({
            object: object,
            property: property,
            oldValue: Color3.FromHexString(_oldValue),
            newValue: Color3.FromHexString(_newValue),
            action: () => {
        ...
        const oldC = Color3.FromHexString(_oldValue).asArray();
        const newC = Color3.FromHexString(_newValue).asArray();
