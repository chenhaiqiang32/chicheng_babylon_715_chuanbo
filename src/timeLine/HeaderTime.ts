...
export class HeaderTime {
    ...
    private installEvents() {
        const isLeftMouseOnly = (e: any) => {
            ...
            const nativeButton = typeof (originalEvent as any)?.button === 'number' ? (originalEvent as any).button : undefined
            const nativeButtons = typeof (originalEvent as any)?.buttons === 'number' ? (originalEvent as any).buttons : undefined
        ...
        this.bg.addEventListener('click', (e: any) => {
            ...
            e.stopPropagation?.();
            e.preventDefault?.();
            const originalEvent = (e as any)?.data?.originalEvent as MouseEvent | PointerEvent | undefined
            if (originalEvent && typeof (originalEvent as any).clientX === 'number') {
                setTimeFromClientX((originalEvent as any).clientX)
            } else {
                setTimeFromPixiEvent(e)
    ...
    setScale(scale: number) {
        ...
        // 背景
        this.bg.clear()
        this.bg.rect(0, 0, this.originX * 2 + this.count * currentUnit, HEADER_HEIGHT).fill({ color: BACKGROUND_COLOR })
        this.bg.zIndex = -1
