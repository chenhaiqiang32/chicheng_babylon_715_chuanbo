import { joinPaths, getDirname } from '@/tools/path';
...
export function createDefaultRenderingPipeline(scene: Scene): DefaultRenderingPipeline {
  ...
  defaultRenderingPipeline.imageProcessingEnabled = false;
...
export function serializeDefaultRenderingPipeline(
  ...
  return {
    ...
    colorGradingWithGreenDepth:
      defaultRenderingPipeline.imageProcessing?.imageProcessingConfiguration
        .colorGradingWithGreenDepth,
    ...
    highlightsSaturation:
      defaultRenderingPipeline.imageProcessing?.colorCurves?.highlightsSaturation,
...
export function parseDefaultRenderingPipeline(data: any, scene: Scene): DefaultRenderingPipeline {
  if (!data) {
    return;
  }
  let defaultRenderingPipeline = scene.postProcessRenderPipelineManager.supportedPipelines.find(
    (x) => x instanceof DefaultRenderingPipeline,
  );
  if (!defaultRenderingPipeline) {
    defaultRenderingPipeline = createDefaultRenderingPipeline(scene);
  }
  ...
  if (defaultRenderingPipeline.imageProcessing) {
    ...
    defaultRenderingPipeline.imageProcessing.vignetteColor = Color4.FromArray(
      data.vignetteColor ?? [0, 0, 0],
    );
    ...
    defaultRenderingPipeline.imageProcessing.colorGradingEnabled =
      data.colorGradingEnabled ?? false;
    defaultRenderingPipeline.imageProcessing.imageProcessingConfiguration.colorGradingWithGreenDepth =
      data.colorGradingWithGreenDepth ?? true;

    if (data.colorGradingTexture && projectConfiguration.path) {
      const rootUrl = joinPaths(getDirname(projectConfiguration.path!), '/');

      let texture: ColorGradingTexture | Texture | null = null;

      if (data.colorGradingTexture.customType === 'BABYLON.ColorGradingTexture') {
        const absoluteUrl = joinPaths(rootUrl, data.colorGradingTexture.name);

        texture = configureImportedTexture(new ColorGradingTexture(absoluteUrl, scene));
    ...
    if (defaultRenderingPipeline.imageProcessing?.colorCurves) {
      ...
      defaultRenderingPipeline.imageProcessing.colorCurves.globalExposure =
        data.globalExposure ?? 0;
      defaultRenderingPipeline.imageProcessing.colorCurves.globalSaturation =
        data.globalSaturation ?? 0;

      defaultRenderingPipeline.imageProcessing.colorCurves.highlightsHue = data.highlightsHue ?? 30;
      defaultRenderingPipeline.imageProcessing.colorCurves.highlightsDensity =
        data.highlightsDensity ?? 0;
      defaultRenderingPipeline.imageProcessing.colorCurves.highlightsExposure =
        data.highlightsExposure ?? 0;
      defaultRenderingPipeline.imageProcessing.colorCurves.highlightsSaturation =
        data.highlightsSaturation ?? 0;

      defaultRenderingPipeline.imageProcessing.colorCurves.midtonesHue = data.midtonesHue ?? 30;
      defaultRenderingPipeline.imageProcessing.colorCurves.midtonesDensity =
        data.midtonesDensity ?? 0;
      defaultRenderingPipeline.imageProcessing.colorCurves.midtonesExposure =
        data.midtonesExposure ?? 0;
      defaultRenderingPipeline.imageProcessing.colorCurves.midtonesSaturation =
        data.midtonesSaturation ?? 0;

      defaultRenderingPipeline.imageProcessing.colorCurves.shadowsHue = data.shadowsHue ?? 30;
      defaultRenderingPipeline.imageProcessing.colorCurves.shadowsDensity =
        data.shadowsDensity ?? 0;
      defaultRenderingPipeline.imageProcessing.colorCurves.shadowsExposure =
        data.shadowsExposure ?? 0;
      defaultRenderingPipeline.imageProcessing.colorCurves.shadowsSaturation =
        data.shadowsSaturation ?? 0;
    }
  }
  ...
  defaultRenderingPipeline.chromaticAberration.direction = Vector2.FromArray(
    data.direction ?? [0, 0],
  );
  defaultRenderingPipeline.chromaticAberration.centerPosition = Vector2.FromArray(
    data.centerPosition ?? [0, 0],
  );
