import { CascadedShadowGenerator, Color3, DirectionalLight, Light, Mesh, AbstractMesh, PointLight, ShadowGenerator, SpotLight } from "@babylonjs/core";
...
export class Shadow {
    ...
    openShadow(light: DirectionalLight | PointLight | SpotLight ,type:'classic'|'cascaded',mapSize?:{width:number,height:number}) {
        if (this.shadowGeneratorMap.has(light.uuid)) {
            return this.shadowGeneratorMap.get(light.uuid);
        }
      //  const shadowGenerator = new ShadowGenerator(1024, light);
      const shadowGenerator =     type === 'classic'
    ...
    removeMeshFromShadowGenerator(mesh: AbstractMesh, light?: DirectionalLight | PointLight | SpotLight) {
        if (light) {
            const shadowGenerator = this.getShadowGenerator(light);
            if (shadowGenerator) {
