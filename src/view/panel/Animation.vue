      ...
      <div class="panel-body" v-if="currentRuntimeAction">
        ...
        <div v-if="selectionCount === 1" class="keyframe-info">
          ...
          <Field title="属性" :showHover="true">
            <ElInput class="readonly" :value="getAnimPropertyLabel(currentSelection?.clip?.property || '')" readonly size="small" style="width:160px" />
          </Field>
          <Field title="帧时间" :showHover="true">
            <ElInput size="small" type="number" style="width:160px" v-model.number="keyTimeLocal" @change="onKeyTimeLocalChange" />
          </Field>
          ...
          <Field title="曲线" :showHover="true">
            <ElCascader
              ...
              :props="easingCascaderProps"
              size="small"
              style="width:160px"
              @change="onEasingLocalChange"
            />
          </Field>

          <!-- rotation (quaternion) 显示为欧拉角（度）并可编辑 -->
          <Field v-if="currentSelection?.clip?.property === 'rotationQuaternion' || currentSelection?.clip?.type === 'quaternion'" title="旋转 (Euler)" :showHover="true" :textWidth="120">
            <div style="display:flex;gap:6px;align-items:center; margin-left:auto;">
              <ElInput size="small" v-model.number="rotationX" type="number" style="width:60px" @change="onRotationLocalChange" />
              <ElInput size="small" v-model.number="rotationY" type="number" style="width:60px" @change="onRotationLocalChange" />
              <ElInput size="small" v-model.number="rotationZ" type="number" style="width:60px" @change="onRotationLocalChange" />
            </div>
          </Field>

          <!-- v3 (position/scale/rotation) -->
          <Field v-if="currentSelection?.clip?.type === 'v3' || ['position','scale'].includes(currentSelection?.clip?.property || '')" title="坐标" :showHover="true">
            <div style="display:flex;gap:6px;align-items:center; margin-left:auto;">
              <ElInput size="small" v-model.number="vectorX" type="number" style="width:60px" @change="onVectorLocalChange" />
              <ElInput size="small" v-model.number="vectorY" type="number" style="width:60px" @change="onVectorLocalChange" />
              <ElInput size="small" v-model.number="vectorZ" type="number" style="width:60px" @change="onVectorLocalChange" />
            </div>
          </Field>

          <!-- color (尝试支持 hex 字符串或数组) -->
          <Field v-else-if="currentSelection?.clip?.property?.toLowerCase().includes('color') || typeof (currentSelection?.key?.value) === 'string'" title="颜色" :showHover="true">
          ...
          <Field v-else-if="typeof (currentSelection?.key?.value) === 'number'" title="值" :showHover="true">
            <ElInput size="small" type="number" style="width:160px" v-model.number="numberLocal" @change="onNumberLocalChange" />
          </Field>
        ...
        <div v-else-if="selectionCount > 1" class="keyframe-info">
          ...
          <Field title="曲线" :showHover="true">
            <ElCascader
              v-model="multiEasingLocal"
              :options="easingOptions"
              :props="easingCascaderProps"
              size="small"
              style="width:160px"
              :placeholder="multiEasingMixed ? '-' : ''"
              @change="onMultiEasingChange"
            />
          </Field>
          ...
          <template v-if="multiCommonProperty">
            ...
            <Field v-if="multiEditorKind === 'quaternion'" title="旋转 (Euler)" :showHover="true" :textWidth="120">
              <div style="display:flex;gap:6px;align-items:center; margin-left:auto;">
                <ElInput size="small" type="text" style="width:60px" v-model="multiRotationXLocal" @change="(v:any)=>onMultiRotationChange('x', v)" />
                <ElInput size="small" type="text" style="width:60px" v-model="multiRotationYLocal" @change="(v:any)=>onMultiRotationChange('y', v)" />
                <ElInput size="small" type="text" style="width:60px" v-model="multiRotationZLocal" @change="(v:any)=>onMultiRotationChange('z', v)" />
              </div>
            </Field>
            ...
            <Field v-else-if="multiEditorKind === 'v3'" :title="multiCommonPropertyLabel" :showHover="true">
              <div style="display:flex;gap:6px;align-items:center; margin-left:auto;">
                <ElInput size="small" type="text" style="width:60px" v-model="multiVectorXLocal" @change="(v:any)=>onMultiVectorChange('x', v)" />
                <ElInput size="small" type="text" style="width:60px" v-model="multiVectorYLocal" @change="(v:any)=>onMultiVectorChange('y', v)" />
                <ElInput size="small" type="text" style="width:60px" v-model="multiVectorZLocal" @change="(v:any)=>onMultiVectorChange('z', v)" />
              </div>
            </Field>

            <!-- color -->
            <Field v-else-if="multiEditorKind === 'color'" title="颜色" :showHover="true">
              <ElInput size="small" type="text" style="width:160px" v-model="multiColorLocal" @change="(v:any)=>onMultiColorChange(v)" />
            </Field>

            <!-- number -->
            <Field v-else-if="multiEditorKind === 'number'" title="值" :showHover="true">
              <ElInput size="small" type="text" style="width:160px" v-model="multiNumberLocal" @change="(v:any)=>onMultiNumberChange(v)" />
            </Field>
          </template>
        </div>
      </div>
...
const currentRuntimeAction = shallowRef<CC.Animation | null>(null)
...
type MultiEditorKind = 'v3' | 'quaternion' | 'color' | 'number' | null
...
watch([selectedKeyframes, multiEditorKind, multiCommonProperty], () => {
  ...
  try {
    ...
    const easingVals = selectedKeyframes.value.map(s => (typeof (s.key as any)?.easing === 'number' ? (s.key as any).easing : 0))
...
function updateSelectedKeyValue(line: number, key: KeyframeData, newValue: any) {
  ...
  if (k) {
    ...
    if (idx !== -1) {
      ...
      clip.key.push({ time: key.time, value: newValue })
      clip.key.sort((a, b) => a.time - b.time)
      added = true
    }
  }
...
const createAnimation = async () => {
  ...
  runtimeAnimations.value.push(newAnim)
  runtimeAnimations.value = [...runtimeAnimations.value]
  Editor.Instance.Scene.runtimeAnimation = toRaw(runtimeAnimations.value)
  Editor.Instance.dispatch('animationChange')
  currentSelect.value = newAnim.uuid
  onSelectChange(newAnim.uuid)
}
