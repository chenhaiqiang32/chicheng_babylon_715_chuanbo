...
import { CollisionMesh, RigidBody } from '@/tools/node/physics';
...
async function createPhysicsTestScene(parentNode?: Node | null) {
  ...
  for (const objConfig of physicsObjects) {
    ...
    if (objConfig.type === 'box') {
      ...
      mesh = MeshBuilder.CreateSphere(objConfig.name, { diameter: objConfig.diameter! }, scene);
    } else {
      continue;
    }
    ...
    // 创建碰撞体
    const collisionMesh = new CollisionMesh(objConfig.name + '_Collision', scene, mesh);
