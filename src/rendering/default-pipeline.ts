import { getExtname, joinPaths, getDirname } from "@/tools/path";

import { DefaultRenderingPipeline, Color4, Vector2, Camera, Texture, ColorGradingTexture } from "@babylonjs/core";
...
export function createDefaultRenderingPipeline(): DefaultRenderingPipeline {
	defaultRenderingPipeline = new DefaultRenderingPipeline("DefaultRenderingPipeline", true, Editor.Instance.Scene, [Editor.Instance.Scene.activeCamera!]);
...
export function parseDefaultRenderingPipeline( data: any): DefaultRenderingPipeline {
	const defaultRenderingPipeline = getDefaultRenderingPipeline() ?? createDefaultRenderingPipeline();
	...
	if (defaultRenderingPipeline.imageProcessing) {
		...
		if (data.colorGradingTexture && projectConfiguration.path) {
			...
			if (data.colorGradingTexture.customType === "BABYLON.ColorGradingTexture") {
				...
				texture = configureImportedTexture(new ColorGradingTexture(absoluteUrl, Editor.Instance.Scene));
				texture.level = data.colorGradingTexture.level;
			} else {
				const parsedTexture = Texture.Parse(data.colorGradingTexture, Editor.Instance.Scene, rootUrl);
